...
const ImageUploader = ({ addImage }) => {
  ...
  const [selectedImage, setSelectedImage] = useState(null);
  const [showUploadedImages, setShowUploadedImages] = useState(false);
  const [loadingUploadIndex, setLoadingUploadIndex] = useState(null); // Track which upload is in progress
  ...
  const handleUploadToCloudinary = async (upload, index) => {
    setLoadingUploadIndex(index); // Set loading state for the current upload
    ...
    try {
      ...
      setLoadingUploadIndex(null); // Reset loading state
    }
  };
        ...
        {uploads.map((upload, index) => (
          <div key={index} className="upload-box">
            <div className="upload-details">
              ...
              <p className='upload-title'>{upload.description}</p>
              <button
                onClick={() => handleUploadToCloudinary(upload, index)}
                className="upload-cloud-button"
                disabled={loadingUploadIndex === index}
              >
                {loadingUploadIndex === index ? 'Uploading...' : 'Upload to Cloudinary'}
              </button>
            </div>
            <div className="upload-image">
              <img src={upload.imagePreview} alt={upload.title} />
